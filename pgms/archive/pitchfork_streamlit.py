...
def run_the_data():
    ...
    sm_df_full = load_full_data('../data/df_genre_stream_wide.csv')
...
def run_the_app():
    ...
    chosen_brewery = st.selectbox('Choose a brewery:', sm_df_full['genre'].unique().tolist())
    
    #dropdown:
    desired_beer = st.selectbox('Choose a beer:', sm_df_full.loc[sm_df_full['genre'] == chosen_brewery]['artist'].unique().tolist())
    
    # cc add
    st.write(f"desired beer: {desired_beer}")
        
    beer_df = sm_df_full[sm_df_full.artist == desired_beer]

    topic = int(beer_df[beer_df.genre == chosen_brewery]['count'].values[0])
    st.write("Here is a look at the dominant words used to describe your choosen beer:")
#     st.image(f'images/topic_{topic}.png')
    st.write(f'Style: {sm_df_full[sm_df_full.artist == desired_beer]["album"].values[0]}')
    ...
    if filter_by == 'Yes':
        beer_location = st.selectbox('Choose a location:', sm_df_full['best'].unique())
    ...
    def full_recommendations(cosine_sim = cosine_sim):
        title = desired_beer # index artist based on full data
        if filter_by == 'Yes':
        ...
        if filter_by == 'No':
            ...
            for i in top_20_indexes:
                ...
                rec_beer_dict['album'] = list(sm_df.artist)[i]
                rec_beer_dict['score'] = sm_df_full.loc[full_ind]['score']
                rec_beer_dict['genre'] = sm_df_full.loc[full_ind]['genre']
...
#                 load_image(img_url)
                st.write(f"{recommended_beers[i]['album']} from {recommended_beers[i]['score']} in {recommended_beers[i]['genre']}")
                st.write("The stats:")
...
#                 rec_beer_dict['avail'] = sm_df_full.loc[full_ind]['avail']
#                 rec_beer_dict['Image'] = sm_df_full.loc[full_ind]['img']
                if sm_df_full.loc[full_ind]['genre'] == location:
                    recommended_beers.append(rec_beer_dict)
